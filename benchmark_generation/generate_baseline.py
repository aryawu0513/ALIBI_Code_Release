#!/usr/bin/env python3
"""Generate one clean, deliberately vulnerable implementation per task bundle."""
...
DEFAULT_TASKS   = HERE / "tasks"
DEFAULT_CONFIG  = HERE / "config_npd.yaml"
MODEL           = "qwen3.6-27b"
...
def main():
    global MODEL
    import argparse
    ap = argparse.ArgumentParser(description="Generate clean vulnerable baseline implementations")
    ap.add_argument("jsonl", help="JSONL metadata with a slug or pilot_id field per task")
    ap.add_argument("ids", nargs="*", help="Slugs to process (default: all)")
    ap.add_argument("--ids-file", help="File with one slug per line")
    ap.add_argument("--tasks-dir", "--samples-dir", dest="tasks_dir", default=str(DEFAULT_TASKS),
                    help="Task-bundle directory containing one directory per slug")
    ap.add_argument("--output-dir",  default=None,
                    help="Directory to write attacker_output.cc (default: --tasks-dir)")
    ...
    samples_dir     = Path(args.tasks_dir)
    ...
    for line in Path(args.jsonl).read_text().splitlines():
        if line.strip():
            row = json.loads(line)
            rows[row.get("slug") or row["pilot_id"]] = row
    ...
    pids = [p for p in pids if (samples_dir / p / "task.md").exists()]
